...
import sys
import platform
from impressoes.forms import LoginForm, DigiForm, AdForm, CriarTutoForm
from impressoes.models import config, tutoriais, logs
    ...
    if dominio: # Dominio existe no banco de dados
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return render_to_response('login.html', {'form': form, 'err': ''}, context_instance=RequestContext(request))
        ...
        return render_to_response('configAdInit.html', {
                                 'title': 'Home',
                                 'form': form,
                                 'itemselec': 'HOME',
                                 },
...
def get_scan(request, printer, arquivo, action): # Se for URL padrão (mostrar pastas)
    ...
    if(printer == '*printer*' and arquivo == '*file*' and action == '*action*'):
        ...
        try:
            ...
            # Descobre o sistema operacional em que o servidor está rodando
            if(platform.system() == 'Windows'): # Se for windows é preciso converter o retorno para latin-1 para não dar problemas com acentos (Saco)
                err = str(sys.exc_info()[1]).decode("latin-1")
            else:
                err = str(sys.exc_info()[1])
        return render_to_response('escaneados.html', {
        ...
        for fili in nomes_arquivos_load:
            if(not(fili[-3::] == 'pdf') and not(fili[-3::] == 'jpg')and not(fili[-3::] == 'tif')and not(fili[-3::] == 'wps') and not(fili[-3::] == 'PDF') and not(fili[-3::] == 'JPG')and not(fili[-3::] == 'TIF')and not(fili[-3::] == 'WPS')and not(fili[-2::] == 'DB')and not(fili[-2::] == 'db')):
                load.append(fili)
        #concatenando arquivos
        nomes_arquivos = load + nomes_arquivos_pdf + nomes_arquivos_jpg
        lista = []
        for fff in nomes_arquivos:
            lista.append(fff.decode("latin-1"))
            ...
            with open(url, 'rb') as pdf:
                if(action == 'dow'): # Forçar Download
                    ...
                    try:
                        ...
                        log = logs(data = datetime.datetime.now(), action = 'Err', item = 'Digitalizacoes/%s/%s' % (str(printer), arquivo), resumo = 'Arquivo não foi removido, err: %s' % (str(err.encode('utf-8'))), user = request.session['userl'], ip = Ip)
                        log.save()
...
def pasta_digi(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            msg = 'Sucesso'
        return render_to_response('pasta_digi.html', {
            'form': form,
...
def dados_ad(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Chama a página novamente
            msg = 'Sucesso'
        return render_to_response('dados_ad.html', {'form': form, 'msg': msg}, context_instance=RequestContext(request))
    # Se não veio algo do POST cria instancia vazia dos formulários com os dados vindos do banco de dados
    else:
        form = AdForm(request, initial={'dominio': model.dominio, 'endservidor': model.endservidor, 'gadmin': model.gadmin, 'ou': model.ou})
        return render_to_response('dados_ad.html', {
            'form': form,
            'itemselec': 'ADMINISTRAÇÃO',
            'msg': msg,
        }, context_instance=RequestContext(request))
...
def add_tutorial(request):
    ...
    if request.method == 'POST':
        ...
        return render_to_response('criar_tutorial.html', {'form': form, 'msg': msg}, context_instance=RequestContext(request))
    # Se não veio algo do POST cria instancia vazia dos formulários com os dados vindos do banco de dados
    else:
        # Cria uma instancia Vazia do form
        form = CriarTutoForm(request, initial={'data': datetime.datetime.today(), 'user': request.session['nome']})
        return render_to_response('criar_tutorial.html', {
            'form': form,
            'itemselec': 'ADMINISTRAÇÃO',
            'msg': msg,
        }, context_instance=RequestContext(request))
    ...
    # Se não veio algo do POST cria instancia vazia dos formulários com os dados vindos do banco de dados
        ...
        if bd: # se houver retorno no bd
            if Action == 'delete':
                ...
                return render_to_response('editar_tutorial.html', {
                    'form': form,
                    'itemselec': 'ADMINISTRAÇÃO',
                    'msg': msg,
                    'id': Id,
            ...
            return render_to_response('criar_tutorial.html', {
                'form': form,
                'itemselec': 'ADMINISTRAÇÃO',
                'msg': 'Tutorial desejado não existe, você pode criar um novo',
                'id': Id,
            }, context_instance=RequestContext(request))
...
def ocr(request, printer, arquivo):
    ...
    try:
        ...
        filename = os.path.join(model.pasta_dig+ '/'+ printer, arquivo)
        ...
        opts = [unicode(filename).encode('utf-8')]
        # convertendo
        ocr.go(opts)
...
def viewlogs(request, user_u, action_u, host_u):
    ...
    resultado = cursor.fetchall()
    # Finalizando renderização da página
    return render_to_response('viewlogs.html', {
                             'title': 'Relatório',
                             'top_users': top_users,
